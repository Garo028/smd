...
function bind(ev) {
    ev.on('chats.set', ({ chats }) => {
        chats.forEach(c => { store.chats[c.id] = c; });
    });
    ...
    ev.on('messages.upsert', ({ messages }) => {
        messages.forEach(msg => {
            ...
            if (store.messages[jid].length > MAX)
                store.messages[jid] = store.messages[jid].slice(-MAX);
        });
    });
}

async function loadMessage(jid, id) {
    return (store.messages[jid] || []).find(m => m.key?.id === id) || null;
}
