...
module.exports = async (sock, chatId, message, args) => {
    try {
        ...
        await reply(sock, chatId, `🔍 Searching Wikipedia...`, message);
        const res = await axios.get(`https://en.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(q)}`,{timeout:10000});
        const d = res.data;
        if (d.type === 'disambiguation') return reply(sock, chatId, `⚠️ *${q}* has multiple meanings. Please be more specific.`, message);
    ...
    } catch(e) { await reply(sock, chatId, `❌ No Wikipedia article found for *${args.join(' ')}*`, message); }
};
