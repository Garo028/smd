...
module.exports = async (sock, chatId, message, args) => {
    ...
    const score=Math.abs(hash)%101; const bar='█'.repeat(Math.round(score/10))+'░'.repeat(10-Math.round(score/10));
    const e=score>80?'💘':score>50?'💕':score>30?'💛':'💔';
    await reply(sock, chatId, `${e} *Love Calculator*\n\n💑 ${names[0]} & ${names[1]}\n\n${bar} *${score}%*\n\n${score>80?'Soulmates! 😍':score>50?'Good match! 💞':score>30?'Could work! 🤔':'Keep looking... 💀'}\n\n_© ScottyMd by Scotty_`, message);
};
